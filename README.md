...
- 节点相当于悬空；
- 它既不是 `0`，也不是 `1`；
- 如果有其他驱动源驱动，节点会由那个驱动源决定；
- 如果所有驱动源都是 `z`，则节点保持为 `z`；
- 如果多个驱动源同时驱动 `0` 和 `1`，则会产生 `x` 或冲突。

---

## 3. 典型用途：三态总线

高阻态最常见的用途是 **三态总线**，让多个模块共享同一根总线，但同一时刻只有一个模块驱动。
...
当 `enable = 1` 时，输出等于 `data_in`；  
当 `enable = 0` 时，输出为 `1'bz`，即高阻态，不驱动总线。

多个这样的三态缓冲器可以接到同一根总线上：
...
- `z` 通常只能用于 **顶层 I/O** 或带有三态资源的接口；
- 在 FPGA/ASIC 内部普通逻辑中，一般不能综合出真正的高阻态；
- 内部信号如果出现 `z`，往往表示设计有问题，比如未连接、多驱动或三态总线使用不当；
- 在仿真中，`z` 可以正常建模；在综合后，三态缓冲器会映射为具体的 IO 单元。
